...
class Proxy(EngineClient):
    ...
    async def _send_request(
        ...
        # Payload encoding with error handling
        try:
            payload = self.encoder.encode(request)
        except Exception as e:
            raise RuntimeError(f"Failed to serialize {request_type.name}") from e
        ...
        start_time = time.perf_counter() if llm_service_envs.TIMECOUNT_ENABLED else None

        # Send request
        await socket.send_multipart(msg, copy=False)

        # Handle the response and update metrics
        async def handle_response(resp):
            if isinstance(resp, Exception):
                raise resp

            # Record time using InstanceGroup attributes
            if llm_service_envs.TIMECOUNT_ENABLED and isinstance(resp, GenerationResponse):
                if hasattr(resp, "proxy_to_worker_time_end"):
                    group.time_count[idx] += 1
                    if start_time is not None:
                        group.time_total[idx] += (resp.proxy_to_worker_time_end - start_time)
            return resp
        ...
        try:
            if expect_stream:
                ...
                # Handle non-streaming response (e.g., Prefill)
                resp = await q.get()
                resp = await handle_response(resp)
                yield resp  # Instead of return, use yield
        finally:
            # Mark request completion
            group.stats_monitor.on_request_completed(idx, request_id=request.request_id)
