...
async def process_prefill_stage(
    ...
    logger.debug(
        "Processing through prefill for req_id: %s/ url: %s", req_id, p_url
    )
    ...
    try:
        ...
        logger.debug(
            "Prefill processing completed successfully for req_id: %s", req_id
        )
...
async def forward_non_stream(
    ...
    logger.debug(
        "Getting response from decode for req_id: %s/ url: %s", req_id, d_url
    )
    headers = {"x-request-id": req_id}
